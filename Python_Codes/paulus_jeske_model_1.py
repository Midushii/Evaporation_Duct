...
target_files.sort()
print(f"Computing duct height for {len(target_files)} Jan/July files...")
...
for year, month, ext, f in target_files:
    try:
        ...
        T2m_K = float(point.t2m.isel({time_dim: SAMPLE_HOUR}).values)
        Td2m_K = float(point.d2m.isel({time_dim: SAMPLE_HOUR}).values)
        SST_K = float(point.sst.isel({time_dim: SAMPLE_HOUR}).values)
        MSLP_hPa = float(point.msl.isel({time_dim: SAMPLE_HOUR}).values) / 100.0
        u10 = float(point.u10.isel({time_dim: SAMPLE_HOUR}).values)
        v10 = float(point.v10.isel({time_dim: SAMPLE_HOUR}).values)
...
df.to_csv("duct_height_by_year.csv", index=False)

print(f"\nSaved: duct_height_by_year.csv")
...
ax.set_xlabel("Year")
ax.set_ylabel("Evaporation Duct Height (m)")
ax.set_title("Evaporation Duct Height Trend, 1940–2025 (Mumbai test point, 19°N 71°E)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("duct_height_trend.png")
plt.show()

print("\nSaved plot: duct_height_trend.png")